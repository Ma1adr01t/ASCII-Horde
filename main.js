...
function hasRequiredElements() {
  return Boolean(
    el.grid && el.hp && el.maxHp && el.shots && el.turn && el.kills && el.enemyCount && el.log && el.inventory &&
...
function generateDungeon() {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const roomStart = { x: rooms[0].cx, y: rooms[0].cy };
    const exitRoom = rooms[rooms.length - 1];
    const exit = { x: exitRoom.cx, y: exitRoom.cy };
    const doors = placeDoors(floors, walls, roomStart, exit);
    const chests = placeChests(floors, doors, roomStart, exit, rooms);
    return { walls, floors, start: roomStart, exit, doors, chests };
  }
  ...
  const fallbackStart = { x: 8, y: 8 };
  const exit = { x: WIDTH - 9, y: HEIGHT - 9 };
  return { walls, floors, start: fallbackStart, exit, doors: [], chests: [] };
}
...
function placeDoors(floors, walls, startPos, exitPos) {
  const doors = [];
  for (const pos of floors) {
    const [x, y] = pos.split(",").map(Number);
    if ((x === startPos.x && y === startPos.y) || (x === exitPos.x && y === exitPos.y)) continue;
...
function isReachable(floors, doors, startPos, target, canOpenLocked) {
  const seen = new Set([key(startPos.x, startPos.y)]);
  const q = [[startPos.x, startPos.y]];
  while (q.length) {
    const [x, y] = q.shift();
...
function placeChests(floors, doors, startPos, exitPos, rooms) {
  const doorSet = new Set(doors.map((d) => key(d.x, d.y)));
  const reachableWithoutKeys = [...floors].filter((p) => {
    const [x, y] = p.split(",").map(Number);
    return isReachable(floors, doors, startPos, { x, y }, false);
  });
  ...
  const pool = reachableWithoutKeys.filter((p) => !doorSet.has(p) && p !== key(startPos.x, startPos.y) && p !== key(exitPos.x, exitPos.y));
  const chests = [];
  ...
  const early = pool.filter((p) => {
    const [x, y] = p.split(",").map(Number);
    return Math.abs(x - startPos.x) + Math.abs(y - startPos.y) <= 14;
  });
...
function setup() {
  ...
  // Avoid re-rendering on pointermove: repainting the whole grid on every hover can lock the UI.
...
function bootGame() {
  ...
  try {
    ...
    showBootError(err && err.message ? err.message : "Unknown startup failure.");
    throw err;
  }
}
